...
$query = $conn->prepare("SELECT ID, firstName, lastName FROM customer WHERE email = ?");
if ($query) {
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION['customer_id'] = $row['ID']; // Set customer_ID in session
    }
    $query->close();
} else {
...
<html>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    env: 'sandbox',
    client: {
        sandbox: 'AbRqcfmpEE7r-g2SdutiHZT8Ecjx6OPXJrJrgVhE9g5LR4OF-6vRI8isN9_EqqocyPQgd-GCyV3V-uNY'
    },
    commit: true,
    style: {
        layout: 'vertical',
    ...
    onAuthorize: function(data, actions) {
        return actions.payment.execute().then(function(payment) {
            ...
            // Fix: Use vanilla JavaScript setValue instead of jQuery val()
    ...
    // PayMongo Button Handling
    const paymongoButton = document.querySelector('.paymongo-button');
       
       paymongoButton.addEventListener('click', function() {
           // Collect necessary order details from the order summary
           ...
           // Collect other necessary form data
           ...
           if (orderType === 'DELIVERY' && !address) {
               alert('Please provide an address for delivery.');
               return;
           }
           ...
           // Send data to the backend to create PayMongo checkout session
...
function showPaymentSection() {
    ...
    // Show payment section and hide confirm button
    paymentSection.style.display = 'block';
    confirmButton.style.display = 'none';

    // Clear and reinitialize PayPal button
    ...
    paypal.Button.render({
        ...
        onAuthorize: function(data, actions) {
            return actions.payment.execute().then(function() {
                ...
                return fetch('store_order_details.php', {
                    method: 'POST',
                    body: formData
                })
                .then(response => response.json())
...
function initializePayPalButton() {
    ...
    paypal.Button.render({
        ...
        payment: function(data, actions) {
            const totalAmount = document.querySelector('[name="payable_amount"]').value;
            return actions.payment.create({
                transactions: [{
                    amount: {
                        total: totalAmount,
...
function processPayment(method, paymentId) {
    ...
    .then(data => {
        ...
        // Important: Use window.location.href for the redirect
        window.location.href = 'order_confirmation.php';
    })
    .catch(error => {
        console.error('Error:', error);
...
function collectOrderItems() {
    ...
    rows.forEach(row => {
        if (row.cells.length >= 4) {  // Ensure row has enough cells
...
function decrementQuantity(button) {
  ...
  if (event.target === confirmOrderModal) {
    ...
    // Show payment section for all order types
    if (orderType) {
        const totalAmount = parseFloat(document.querySelector('.total-amount').textContent.replace('Php ', ''));
        const serviceFee = totalAmount * 0.05;
        const payableAmount = totalAmount + serviceFee;
        ...
        // Add scroll functionality for all order types
        const modalContent = document.querySelector('.modal-content');
